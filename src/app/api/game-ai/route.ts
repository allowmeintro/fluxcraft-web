...
ДЕРЕВО РЕШЕНИЙ
═══════════════════════════════════════════════════
1. Запрос содержит биом/ландшафт/мир/природу?  → generate_map (ВСЕГДА!)
2. «замени ВСЕ [тип] на [тип]»?                → patch_tiles
3. «измени область / часть / регион»?          → modify_area
4. «поставь рядом со мной»?                    → place_objects
5. «построй структуру»?                        → build
...
function isBuildRequest(prompt: string): boolean {
  const t = prompt.toLowerCase();
  return /(построй|построить|постройте|возведи|сооруди|строй|создай)\s+\w*(замок|дом|башн|крепост|форт|стен|здание|постройк)/.test(t)
    || /\b(замок|крепость|форт)\b/.test(t) && /(построй|создай|сделай|возведи)/.test(t)
    || /(построй|возведи|сооруди)\s+(замок|дом|башн|крепост|форт|стен)/.test(t);
}
